...
class InitializeMarketplaceUsers extends Simulation {
  ...
  val initAdminUsers = scenario("Initializing " + adminCount + " admin users.")
    .feed(Feeders.generateUserName(isAdmin = true))
    .exec(createUser)

  val initUsers = scenario("Initializing " + userCount + " non-admin users")
    .feed(Feeders.generateUserName())
    .exec(createUser)

  setUp(
    initAdminUsers.inject(atOnce(1 user), nothingFor(30 seconds), ramp((adminCount - 1) users) over ((adminCount -1) seconds)),
